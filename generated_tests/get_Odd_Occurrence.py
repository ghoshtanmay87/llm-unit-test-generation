...
def test_single_element_array_returns_that_element():
    ...
    arr_size = 1
    expected = 7
    assert get_Odd_Occurrence(arr, arr_size) == expected
...
def test_array_with_last_element_having_odd_occurrence_returns_that_element():
    arr = [5, 5, 6, 6, 7]
    arr_size = 5
    expected = 7
    assert get_Odd_Occurrence(arr, arr_size) == expected

def test_array_with_multiple_elements_but_only_one_odd_occurrence_element():
    arr = [9, 9, 9, 8, 8, 7, 7]
    arr_size = 7
    expected = 9
    assert get_Odd_Occurrence(arr, arr_size) == expected